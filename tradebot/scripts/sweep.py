...
from src.ai.features import build_feature_frame
from train_from_csv import frame_to_quotes, load_csv_directory
...
def run(symbol: str, csv_dir: str, names: list[str], indicator_config,
        overrides_map: dict) -> None:
    print("Chargement des données...")
    raw = load_csv_directory(Path(csv_dir), symbol)
    quotes = frame_to_quotes(raw)
    ...
    for i, name in enumerate(names, start=1):
        results.append(_train_one(features, name, overrides_map[name], symbol,
                                  indicator_config, i, len(names)))
        print()
    _summary(results)


def run_indicators(symbol: str, csv_dir: str) -> None:
...
def main() -> None:
    ...
    if args.indicators:
        run_indicators(args.symbol, args.csv_dir)
        return
    ...
    run(args.symbol, args.csv_dir, names, IndicatorConfig(), VARIANTS)
