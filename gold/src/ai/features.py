...
def active_feature_columns(use_volume: bool = True) -> list:
    """Colonnes réellement données au modèle.

    Sur l'or au comptant et le forex, il n'existe pas de volume consolidé :
    la colonne serait constante. Une feature constante n'apporte aucune
    information et occupe une entrée du réseau pour rien.
    """
    if use_volume:
        return list(FEATURE_COLUMNS)
    return [c for c in FEATURE_COLUMNS if c != "log_volume_ratio"]
...
def build_feature_frame(quotes: List[Quote],
    ...
    for k in range(total - 1):          # -1 : la dernière bougie n'a pas de cible
        ...
        if use_volume:
            ...
            log_volume_ratio = (
                np.log1p(quote.volume) - np.log1p(mean_volume)
                if mean_volume > 0 else 0.0
            )
            # Écrêtage : sur les bougies hors séance, le volume peut tomber à
            # quasi zéro et produire un ratio de -10 ou pire. Cette queue
            # asymétrique domine le gradient et apprend au modèle à
            # reconnaître les heures creuses plutôt qu'à prédire le prix.
            log_volume_ratio = float(np.clip(log_volume_ratio, -3.0, 3.0))
        else:
