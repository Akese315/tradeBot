...
def set_seed(seed: int) -> None:
    """Fixe toutes les sources d'aléa de l'entraînement.

    Trois générateurs interviennent :
      - `random`  : rien de critique ici, mais fixé par principe
      - `numpy`   : initialisations et découpages
      - `torch`   : poids initiaux, dropout, ordre de mélange du DataLoader
        (`torch.manual_seed` couvre CUDA et MPS en plus du CPU)

    La reproductibilité n'est pas parfaite sur GPU — certaines opérations
    sont non déterministes par nature — mais elle est largement suffisante
    pour comparer des configurations.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def train(splits: SplitData, config: ModelConfig | None = None,
          ...
          indicator_config=None, context: Dict | None = None,
    ...
    device = get_device()
    print(f"Périphérique utilisé : {device} | graine {config.seed}")
    ...
    model = TradingModel(
        # Lu sur les splits et non sur la constante : sur l'or, la feature
        # de volume est absente, donc 11 entrées au lieu de 12.
    ...
    if return_predictions:
        # Stockées sous des clés que run_log n'écrit pas dans le journal :
        # un tableau de 2000 flottants n'a rien à faire dans un fichier texte.
        predictions, true_values = collect_predictions(model, test_loader, device)
        report["predictions"] = predictions
        report["targets"] = true_values
...
def collect_predictions(model: nn.Module, loader: DataLoader,
                        device: torch.device):
    """Récupère les prédictions du test, DANS L'ORDRE.

    L'ordre est essentiel : il permet de rattacher chaque prédiction à sa
    ligne du DataFrame, donc à son régime de marché. C'est possible parce que
    le DataLoader de test n'est PAS mélangé (`shuffle=False` par défaut) —
    contrairement à celui d'entraînement.
    """
    model.eval()
    predictions, targets = [], []
    with torch.no_grad():
